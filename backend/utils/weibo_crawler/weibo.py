...
# 日期时间格式
TIMEZONE = timezone(timedelta(hours=8)) # 中国标准时间
...
class Weibo(object):
    def __init__(self, config):
        ...
        if isinstance(since_date, int):
            since_date = datetime.now(TIMEZONE).replace(tzinfo=None) - timedelta(since_date)
    ...
    def standardize_date(self, created_at):
        """标准化微博发布时间"""
        now_in_china = datetime.now(TIMEZONE).replace(tzinfo=None)
        ts = None

        if "刚刚" in created_at:
            ts = now_in_china
        elif "分钟" in created_at:
            minute = int(created_at[: created_at.find("分钟")])
            ts = now_in_china - timedelta(minutes=minute)
        elif "小时" in created_at:
            hour = int(created_at[: created_at.find("小时")])
            ts = now_in_china - timedelta(hours=hour)
    ...
    def get_pages(self):
        ...
        try:
            ...
            if since_date <= today:    # since_date 若为未来则无需执行
                ...
                self.start_date = today.strftime(DTFORMAT)
                ...
                if len(info) > 0 and info[0].isdigit():
                    ...
                    if len(info) == 3:
                        if self.is_datetime(info[2]):
                            ...
                            since_date = datetime.now(TIMEZONE).replace(tzinfo=None) - timedelta(int(info[2]))
